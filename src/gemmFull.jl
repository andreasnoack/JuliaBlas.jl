...
#
    ...
    # register clobber list
        "~{rax},~{rbx},~{rcx},~{rdx},~{rsi},~{rdi},~{r8},~{r9},~{r10},~{r11},~{xmm0},~{xmm1},~{xmm2},~{xmm3},~{xmm4},~{xmm5},~{xmm6},~{xmm7},~{xmm8},~{xmm9},~{xmm10},~{xmm11},~{xmm12},~{xmm13},~{xmm14},~{xmm15}\""*
        "(i64 %0, i64 %1, double* %2, double* %3, double %4, double %5, double* %6, i64 %7, i64 %8)"
...
@inline function gemm_micro_kernel(kc::Integer, α::Float64, A::Array{Float64}, Aoffset::Integer,
    ...
    @inbounds begin
        ...
        Base.llvmcall("""
            ...
            ret void""",
        Void,
        Tuple{Clong, Clong, Ptr{Float64}, Ptr{Float64}, Float64, Float64, Ptr{Float64}, Clong, Clong},
        Clong(kb), Clong(kl), pointer(A, Aoffset + 1), pointer(B, Boffset + 1), α, β, pointer(C, Coffset + 1), Clong(incRowC), Clong(incColC))
    end
end
